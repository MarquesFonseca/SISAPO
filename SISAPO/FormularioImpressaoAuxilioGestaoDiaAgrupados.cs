...
namespace SISAPO
{
    public partial class FormularioImpressaoAuxilioGestaoDiaAgrupados : Form
    {
        private DataTable listaObjetos;
        private DataTable DtTempPCT = new DataTable();
        private DataTable DtTempPCTINT = new DataTable();
        private DataTable DtTempENV = new DataTable();
        private DataTable DtTempCAIXAPOSTAL = new DataTable();
        private DataTable DtTempACOBRAR = new DataTable();

        public FormularioImpressaoAuxilioGestaoDiaAgrupados(DataTable _listaObjetos)
        {
            InitializeComponent();
        ...
        private void FormularioImpressaoAuxilioGestaoDiaAgrupados_Load(object sender, EventArgs e)
        {
        ...
        private StringBuilder RetornaHtml()
        {
            ...
            #region IniciaGrupos PCT
            Html = CarregaHTMLTipoComentario(Html, DtTempPCT, "TIPO: PCT");
            #endregion

            #region IniciaGrupos PCT INT
            Html = CarregaHTMLTipoComentario(Html, DtTempPCTINT, "TIPO: PCT INTERNACIONAL");
            #endregion

            #region IniciaGrupos CAIXA POSTAL
            Html = CarregaHTMLTipoComentario(Html, DtTempCAIXAPOSTAL, "TIPO: CAIXA POSTAL");
            #endregion

            #region IniciaGrupos A COBRAR
            Html = CarregaHTMLTipoComentario(Html, DtTempACOBRAR, "TIPO: A COBRAR");
            #endregion

            #region IniciaGrupos ENVELOPE
            Html = CarregaHTMLTipoComentario(Html, DtTempENV, "TIPO: ENVELOPE");
            #endregion
        ...
        {            
            if (Dt.Rows.Count > 0)
            {
                ...
                Html.AppendLine("	<tbody>		");
                Html.AppendLine("		<tr>");
                Html.AppendLine("		    <td width=\"690\" bgcolor=\"#F3F3F3\" style=\"text-align:left\"><b><font size=\"6\">" + DescricaoTiopo + "</font></b></td>");
                Html.AppendLine("		    <td width=\"80\" bgcolor=\"#F3F3F3\" style=\"text-align:right\"><b><font size=\"3\">TOTAL:</font></b></td>");
                Html.AppendLine("		    <td width=\"80\" bgcolor=\"#F3F3F3\" style=\"text-align:center\"><b><font size=\"6\">" + qtd + "</font></b></td>");
                Html.AppendLine("		</tr>");
                Html.AppendLine("	</tbody>");
                Html.AppendLine("	</table>");
                ...
                foreach (DataRow dr in Dt.Rows)
                {
                    ...
                    Html.AppendLine("		    <td>" + Convert.ToDateTime(dr["DataLancamento"]).ToShortDateString() + "</td>");
